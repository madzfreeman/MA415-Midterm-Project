...
```
Here is my analysis of the different production types (organic and conventional). First I began by creating a table for production comparison, then sales comparison and then processing comparison. After this I created bar plots to exhibit the production comparison and processing comparison. I tried to make the bar graphs as clear as possible by including keys, labels and bright colors.
```{r}
#| echo: false
...
create_plot <- function(data, y_col, title) {
  ggplot(data, aes(x = `Production.Type`, y = .data[[y_col]], fill = `Production.Type`)) +
    geom_col() +
    labs(title = title, x = "Production Type", y = y_col) +
    scale_fill_manual(values = c("Conventional" = "orange", "Organic" = "pink")) +
    theme_minimal()
}
create_plot(prod_summary, "Total_Production", "Total Production Comparison")
create_plot(processing_summary, "Processing_Amount", "Processing Amount Comparison")
```
